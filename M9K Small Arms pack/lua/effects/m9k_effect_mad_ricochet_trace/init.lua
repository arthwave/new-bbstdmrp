...
function EFFECT:Init(data)
	...
	-- FIX: If EndPos and StartPos are the same, calculate EndPos from Dir
	if self.EndPos:DistToSqr(self.StartPos) < 100 then  -- Less than 10 units apart
		-- Try to get direction from elsewhere
		local dataDir = data:GetAngles()  -- This might have direction encoded
		-- Fallback: use a default distance in an arbitrary direction
		self.EndPos = self.StartPos + Vector(100, 0, 0)  -- Temporary fallback
	end
